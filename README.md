...
| `v` | **段评侧栏**:开/关,显示当前章节各段落点赞数最高的 10 条评论 |
| `←` / `→` | 段评侧栏:切换到上/下一个有评论的段落 |
| `c` | 章节目录 |
| `q` | 返回书架 |
| `Ctrl+S` | 打开设置 |
...
阅读时按 `v` 打开右侧段评栏,**自动同步到当前正在阅读的段落**:
根据滚动位置估算当前段落,找到最近的有评论的段落,按点赞数降序显示 top 10 评论
(用户名 + ❤点赞数 + 正文)。再次按 `v` 时,若当前阅读段落与侧栏显示不同则重新同步,
相同则关闭。`←` / `→` 手动切换其他有评论的段落。需本地 unidbg 服务运行中。
正常模式和伪装模式均可使用。

**段落指示符**:章节加载时后台预加载段评统计,滚动阅读时底部状态栏显示
不显眼的 `¶N→M ♥C` 标志(N=当前段落,M=最近的段评段落,C=评论数),
让你一眼看到按 `v` 会打开哪个段落的评论。伪装模式下标志用灰色(grey53)显示。
